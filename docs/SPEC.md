...
最終更新: 2026-07-15 / 対象: `main`（PR #1〜#21 反映済み。Material 3 リデザイン＋開運UIUX Phase A 適用）
...
- **カラー**: primary＝**瑠璃**（七宝の青・light `#1e50a2`/dark `#adc6ff`）、tertiary＝金箔由来のゴールド（吉・強調の要所のみ）、good/caution＝M3トーンの吉/凶（caution が M3 error 役を兼務）。サーフェスは `--surface-container-*` 階層（生成り/夜紺トーン）。カードは3層＝hero（`.card`＋`--elev-1`）／filled（`.card-filled`・影なし）／outlined。
...
6. **デザイントークンを変えたら同期を確認**：ライト定義2ブロック（`@media` 内と `[data-theme="light"]`）・`viewport.themeColor`（layout.tsx）・`--bg-hi/--bg-lo`。金の作法（常設3箇所まで・caution に金を載せない・操作色は瑠璃）を崩さない。
