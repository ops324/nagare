...
**デザインは Material 3 準拠**（トークンは `app/globals.css` に集約）：
...
- **カラー**: primary＝紺青由来のブルー、tertiary＝金箔由来のゴールド（吉・強調の要所のみ）、good/caution＝M3トーンの吉/凶（caution が M3 error 役を兼務）。サーフェスは `--surface-container-*` 階層、カードは elevated card（`--elev-1`）。
- **ナビ**: 画面下部の M3 ナビゲーションバー（`components/NavBar.tsx`・5項目・アイコン自作SVG・`nav`＋`aria-current` セマンティクス）。
- **タイポグラフィ**: 本文/UI＝Noto Sans JP、ワードマーク・大きな漢字値（干支・宿名・命式柱等）＝Shippori Mincho（和の要所）。
