...
最終更新: 2026-08-01 / 対象: `main`（PR #1〜#46 反映済み）
...
| daiun.test | 大運（順逆・立運・初運）・**節入り日生まれの立運**（月柱と同じ境界／「同じ月柱なら順行＋逆行の和が一致」の性質テスト＝10年ズレの回帰テスト） |
...
- **精査（#41〜#45）で見つけたが、正しさに直接影響しないため未着手の3点**：
  - `daiun.ts` の `DaiunPeriod.ageStart` が `startMonths` を捨てる（ヘッダの「概算」どおりだが、`DaiunList` の現在期の強調が境界付近で最大1年ずれうる。**ここだけ概算値を表示ではなく比較に使っている**）。
