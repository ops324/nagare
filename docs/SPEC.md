...
最終更新: 2026-07-16 / 対象: `main`（PR #1〜#23 反映済み。Material 3 リデザイン＋開運UIUX Phase A 適用、操作色は金基調）
...
**デザインは Material 3（M3 Expressive）× 煌ぐ和モダン**（トークンは `app/globals.css` に集約）。基準イメージ＝「生成りの光の上に金のゲージ・明朝の数字・常に金のラベル」（運が低い日も美しい）：
- **テーマ**: ライト/ダーク自動切替（`prefers-color-scheme` 連動、`:root[data-theme]` で上書き可。ライト定義は media クエリ内と `[data-theme="light"]` の2ブロックを同期維持）。`color-scheme` をテーマ駆動し、ネイティブ日付/時刻ピッカーも追随。
- **背景「生成りの光」**: ライト＝生成り（`#fdfaf1→#f6f0e0`）、ダーク＝夜紺（`#161828→#101220`）の上下グラデーション（`body::before` 固定層、トークン `--bg-hi/--bg-lo`）。
- **カラー**: primary＝**金**（金箔・light `#715c0f`/dark `#dbc66f`）。tertiary は primary と同値の同系ゴールド（`--accent` が primary の別名、`--accent-soft` は一段ずらしたトーン＝dark `--gold-300`／light `--gold-500`。生成り地では `--gold-300` が 4.2:1 で AA 未達のため、ライトは暗い側へ振る）。good/caution＝M3トーンの吉/凶（caution が M3 error 役を兼務）。サーフェスは `--surface-container-*` 階層（生成り/夜紺トーン）。カードは3層＝hero（`.card`＋`--elev-1`）／filled（`.card-filled`・影なし）／outlined。
...
- **開運レイヤー**: ①「今日の色」＝日干の五行→伝統色（`components/lucky.ts`、`html[data-lucky]`＋`--lucky*` トークン、方式は `PROVENANCE.luckyColor`）。適用はひとことカードと開運アクションの2ゾーンのみ ②ひとこと（`components/Hitokoto.tsx`＝挨拶＋トップシグナル＋`BAND_COPY` のテンプレ合成・非LLM、ストリーク・命の日バッジ） ③開運アクション（`components/LuckyActions.tsx`＝シグナル優先度で `ACTION_COPY` から最大3件・朗報ファースト） ④祝祭モーション（スプリングトークン `--spring-*`、天赦日の金の粒バースト＝日1回ガード＋reduced-motion 抑止、大安の脈動、score≥78 の金ブルーム。ゲージは弧・光暈・ラベルまで常に金の聖域）。
- **ナビ**: 画面下部の M3 ナビゲーションバー（`components/NavBar.tsx`・5項目・アイコン自作SVG・`nav`＋`aria-current` セマンティクス・活性ピルのスプリングモーフ）。
...
5. 表示に関わるなら **Claude Preview で実機確認**（ライト/ダーク・スマホ 375px・今日の色は `data-lucky` 5色）。
6. **デザイントークンを変えたら同期を確認**：ライト定義2ブロック（`@media` 内と `[data-theme="light"]`）で `--primary`/`--accent-soft`/`--weekday-*` を揃える・`viewport.themeColor`（layout.tsx）・`--bg-hi/--bg-lo`。金の作法（操作色は金・caution に金を載せない・ライトでは金を暗い側のトーンへ振って AA 4.5:1 を確保）を崩さない。`--weekday-sat/sun` は暦の慣習用で操作色から独立（§7）。金を別の色へ振り直す場合は**同系の金が重なる 2 グラフ**（`Biorhythm.tsx` の からだ/知性、`LifeTimeline.tsx` の帯とノード）で線が判別できるか必ず確認する。
...
日干支/干支＝十干十二支の60周期。節月＝節気で区切る月（立春=寅月）。回座＝九星が年盤で巡る位置。八方塞がり＝本命星が中宮に入る年。空亡/天中殺＝日柱の旬で余る2支。運命星/星人＝六星占術の分類。大殺界＝六星の運気の陰影・停止・減退。本命宿＝生時の月が宿る27宿。三九の秘法＝宿曜の相性。二十八宿＝暦の日替わりの宿（牛宿を含む28）。今日の色＝日干の五行（甲乙木・丙丁火・戊己土・庚辛金・壬癸水）を日本の伝統色（萌黄・紅・琥珀・金箔・浅葱）に写した日替わりアクセント。命の日＝今日の27宿が本命宿と重なる個人の吉日（`todayShuku().isMeinichi`）。
