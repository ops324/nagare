...
| PR | 内容 |
|---|---|
| #50 | 意匠「硝子と箔」＝硝子を**限定素材**として定義（クローム3層＋ひとこと・許可リストをテストで固定）／**配信CSSから `backdrop-filter`（unprefixed）が消えていた欠陥を修正**＝Firefox でヘッダー・ナビ・トーストが「ぼかし無しの半透明」だった／非対応ブラウザ向けの不透明退避／`view()` によるスクロール連動リビール（JS 追加なし）／hover の応答 |
...
- **意匠「硝子と箔」**（PR #50・方針は [`design.md`](../design.md) が正本）: PR #34 の「ガラス → 紙」は**本文の面**についての立場であって素材の否定ではなかった、と読み直し、素材の法を一文に固定した ——**硝子は読み手と本文のあいだに割り込む層のもの、紙は本文そのもの**。ヘッダー・ナビ・トーストが硝子なのは本文の上を通るからで、カードが紙なのはそれが本文だから。
  ...
  - **階層は三段**（design.md）: 主＝フローメーター／ひとこと・副＝カード・地＝凡例と注記。**主を大きくする方向で作る**（副や地を沈めない）— `--text-faint` は明るい地で 4.0:1 しかなく小さな字では AA を割るため、静けさをコントラストで買わない。
