...
改修後、以下が変わったら**バグの疑い**。すべて Vitest（`npm test`・245件）で固定済み。
...
`npm test`（Vitest・245件）。**参照値テスト＝§5の不変条件を固定**。改修時は必ず緑を維持。
...
4. `npm test`（245件）→ `tsc --noEmit` → `eslint .` → `npm run build` を**すべて緑**に。
