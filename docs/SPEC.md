...
| 立春・節入りの粒度 | **瞬間**（暦日でない）。立春2000=2/4 21:40 JST → 2/4 10:00 は立春年1999・一白水星・己卯年・丁丑月・火星人−／23:00 は2000・九紫火星・庚辰年・戊寅月・火星人＋。啓蟄2026=3/5 22:58 → 3/5 10:00 は庚寅月・順行立運0年、23:30 は辛卯月・順行立運10年4ヶ月。**選日だけは暦日基準**（2026-03-05 は0時/10時/23時いずれも 天赦日・一粒万倍日・寅の日） |
...
| 六星占術 ±の境界 | **立春の瞬間**（PR #44）。1990-02-03=天王星人−／02-04（正午）=天王星人＋（同一星人で±のみ反転。立春1990=2/4 11:13 なので 2/4 09:00 生まれは −）。1985-01-20=水星人＋で大殺界2031〜2033（暦年基準だと−になり2032〜2034へ1年ずれる）。**流派差あり・一次資料未確認**＝暦年説の対抗値（2032〜2034）もテストで固定済み（§6・§10） |
...
- **±の年境界（PR #37・境界の粒度は #44）**：`unmeisei` は `risshunYear(birthInstant)` を使う。ここだけ暦年にすると、立春前生まれの ± が反転し `off`（＋=空亡第1支+2／−=+3）が1ずれるため、**運気サイクルが丸ごと1年ずれて大殺界の3年が後ろへ動く**（実報告あり）。運気の年（`runkiForYear` に `risshunYear(now)` が入る）と同じ境界に必ず揃えること。境界は**立春の瞬間**なので、立春当日生まれは出生時刻で ± が変わる（`provenance.rokusei.version = 2026-anchor-v3`）。
...
- **`nextDaisakkai` は大殺界中なら `nineYear + 3` から探す（PR #43）**。大殺界は陰影→停止→減退の**連続3年**なので、`+1` だと今いる組の2年目を「次」として出してしまう。`+3` で次の組の頭（陰影）が返る。
...
`npm test`（Vitest・247件）。**参照値テスト＝§5の不変条件を固定**。改修時は必ず緑を維持。

| テスト | 守っている対象 |
|---|---|
| koyomi.test | 節気・日干支・旧暦・六曜・選日・**節入り日の選日は時刻によらず暦日で決まる**（`setsugetsuBranchOfDay` を瞬間基準へ動かす改修を止めるガード） |
| kyusei.test | 本命星・立春・回座・八方塞がり・**立春の境界は暦日でなく瞬間**（2000/2025/2026/1990 の立春前後で本命星と六星±が反転） |
...
- タイムゾーン: **JST固定**。年境界: 命術は**立春**（九星・四柱・**六星の運気と±**）、厄年は**数え年（元日基準）**。
- **立春・節入りの年/月境界は「瞬間」で解決する（PR #44）**。立春当日でも立春時刻より前は前年、節入り日でも節入り時刻より前は前の節月。出生時刻を入れれば反映される（未入力は正午が代表点）。**ただし選日（一粒万倍日・天赦日）だけは暦日基準**＝`koyomi.setsugetsuBranchOfDay`（節入り日はその日全体が新節月）。暦注は暦日の概念であり、ここは妥協ではなく定義。※六星の±はかつて暦年基準だったが、運気年と食い違い立春前生まれの大殺界が1年ずれるため立春基準へ統一（PR #37・§6）。
...
4. `npm test`（247件）→ `tsc --noEmit` → `eslint .` → `npm run build` を**すべて緑**に。
