...
占術ロジックは参照値テスト（`npm test`・465件）で不変条件を固定している。基盤（`lib/time.ts` `lib/koyomi.ts` `lib/astro.ts` `lib/profile.ts` `lib/flow.ts`）ほど広く波及するため、変更時は影響先を必ず再検証する。実装を変えたら `docs/SPEC.md` も更新すること。
