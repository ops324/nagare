...
const SENJITSU_ACTIONS: { key: string; glyph: string }[] = [
  { key: 'tensha', glyph: '赦' },
  { key: 'ichiryu', glyph: '萬' },
  { key: 'tora', glyph: '寅' },
  { key: 'tsuchinotomi', glyph: '巳' },
  { key: 'mi', glyph: '巳' },
  { key: 'kinoene', glyph: '甲' },
];
...
function pickActions(today: TodayFlow): Pick[] {
  const out: Pick[] = [];
  const add = (key: string, glyph: string, basis: string, end?: string) => {
    const c = ACTION_COPY[key];
    if (!c) return;
    const title = end ? c.title.replace('{end}', end) : c.title;
    if (out.some((p) => p.title === title)) return; // 巳/己巳の重複など
    out.push({ key, glyph, basis, title, note: c.note });
  };

  const senjitsu = today.data.senjitsu;
  for (const sa of SENJITSU_ACTIONS) {
    const hit = senjitsu.find((s) => s.key === sa.key && s.tone === 'good');
    if (hit) add(sa.key, sa.glyph, hit.name);
  }

  const m = today.data.moon;
  if (m.phaseIndex === 0) add('newmoon', '朔', '新月');
  if (m.phaseIndex === 4) add('fullmoon', '望', '満月');
  if (today.data.rokuyo.index === 0) add('taian', '安', '大安');
  if (m.phaseIndex !== 0 && m.phaseIndex !== 4) {
    if (m.waxing) add('waxing', '満', '満ちる月');
    else add('waning', '欠', '欠ける月');
  }
  if (today.data.voidOfCourse.isVoid) {
    add('void', '空', '月のボイドタイム', jstHm(today.data.voidOfCourse.signChange));
  }
  const b = today.data.biorhythm;
  if ((b.physical + b.emotional + b.intellectual) / 3 < -0.3) add('biolow', '憩', 'バイオリズム');
  if (today.data.mercuryRetrograde) add('retro', '☿', '水星逆行');
  add('base', '整', '今日の整え');
