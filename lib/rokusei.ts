 ...
 * 31-40天王星人／41-50木星人／51-60水星人。±は生まれ年（立春基準）の十二支の陰陽。
 * 実例 1985-08-15（星数23＝火星人, 乙丑年＝陰支）→「火星人マイナス」で検証。
 *
 * ±の年境界は立春。1/1〜立春前生まれは前年扱いとし、運気の年（runkiForYear）と同じ境界に揃える。
 * （暦年基準だと立春前生まれだけ±が反転し、運気サイクルが1年ずれて大殺界の3年がずれる）
 *
...
export function unmeisei(birthInstant: Date): Unmeisei {
  ...
  // ±は「立春基準の生まれ年」の十二支の陰陽（陽支=子寅辰午申戌=偶数index / 陰支=奇数index）
  const branch = mod(risshunYear(birthInstant) - 4, 12); // 立春年の年支（甲子年=4AD）
